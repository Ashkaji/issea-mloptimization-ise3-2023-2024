...
def adagrad_optimizer(X: np.ndarray, y: np.ndarray, X_val: np.ndarray, y_val: np.ndarray, learning_rate: float = 0.1, n_iter: int = 1000, epsilon: float = 1e-8, tolerance: float = 1e-6) -> Tuple[np.ndarray, List[float], List[float], float]:
    """
    Implémente l'algorithme Adagrad (Adaptive Gradient Algorithm).
    ...
    Args:
        ...
        learning_rate (float): Taux d'apprentissage. Par défaut à 0.1.
    ...
    accumulated_grad_squared = np.zeros_like(weights)
    train_losses = []
    val_losses = []
