...
def gradient_descent(X: np.ndarray, y: np.ndarray, X_val: np.ndarray, y_val: np.ndarray, learning_rate: float = 0.01, n_iter: int = 1000, tolerance: float = 1e-6) -> Tuple[np.ndarray, List[float], List[float], float]:
    """
    Implémente la descente de gradient pour la régression logistique.
